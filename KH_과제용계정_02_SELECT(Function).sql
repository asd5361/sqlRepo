...
SELECT SUBSTR(professor_name,2,2) AS "이름" FROM tb_professor WHERE professor_name LIKE '___';
...
SELECT TRUNC(AVG(POINT),1) "평점" FROM tb_GRADE WHERE student_no LIKE 'A517178';
...
SELECT SUBSTR(term_no,1,4) "년도" , ROUND(AVG(POINT),1)"년도 별 평점" FROM tb_GRADE WHERE student_no LIKE 'A112113' GROUP BY SUBSTR(term_no,1,4) ORDER BY 1;
...
SELECT department_no,COUNT(DECODE(ABSENCE_YN, 'Y', 1, NULL)) AS "휴학생 수" 
